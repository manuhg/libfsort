...
public:
    ...
    vector<vector<double>>  run(vector<vector<float>> data, int op_dims=3, int max_iters=1000, int num_threads=2,double perplexity=30, double theta=0.5,double amplify=1000.0, bool normalize=false);
...
vector<vector<double>> extract_embeddings(vector<string> image_files,int perplexity=30,int threads=3,bool print_tsnepts=false,float amplify=1000.0,bool normalize=false);
